...
async function main(buildDir?: string, outDir?: string): Promise<void> {
	...
	const appSizeBytes = getDirectorySize(appPath);
	const dmgSizeGb = Math.max(2, Math.ceil(appSizeBytes / (1024 ** 3)) + 1);
	const dmgSize = `'${dmgSizeGb}g'`;

	console.log(`Creating DMG for ${product.nameLong}...`);
	console.log(`  App path: ${appPath}`);
	console.log(`  Output directory: ${outDir}`);
	console.log(`  DMG size: ${dmgSizeGb} GB`);
