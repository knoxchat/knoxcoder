...
function skuFromAccount(account: IDefaultAccount | null): InlineSuggestSku | undefined {
	if (account?.entitlementsData?.access_type_sku && account?.entitlementsData?.subscription_plan) {
		return { type: account.entitlementsData.access_type_sku, plan: account.entitlementsData.subscription_plan };
	}
	return undefined;
}
