...
export abstract class AbstractTaskService extends Disposable implements ITaskService {
	...
	private async _handleLongRunningTaskCompletion(event: ITaskProcessEndedEvent | ITaskInactiveEvent, durationMs: number): Promise<void> {
		...
		if (taskRunSource === TaskRunSource.AssistAgent) {
			return;
		}
				...
				if (active) {
					this._terminateAll().then((responses) => {
						...
						if (response.code && response.code === TerminateResponseCode.ProcessNotFound) {
							this._notificationService.error(nls.localize('TerminateAction.noProcess', 'The launched process doesn\'t exist anymore. If the task spawned background tasks exiting KnoxCoder might result in orphaned processes.'));
						} else {
