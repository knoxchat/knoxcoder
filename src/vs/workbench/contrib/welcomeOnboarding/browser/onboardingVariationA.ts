...
type InstallChatEvent = any;
type InstallChatClassification = any;
enum ChatSetupStrategy { DefaultSetup = 0, DefaultSetupWithoutWorkspaceTrust = 1, SetupWithEnterpriseProvider = 2 }
...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		this._createInlineLink(copilotDisclaimer, localize('onboarding.signIn.disclaimer.settings', "settings"), this.defaultAccountService.resolveGitHubUrl(GitHubPaths.featureSettings));
