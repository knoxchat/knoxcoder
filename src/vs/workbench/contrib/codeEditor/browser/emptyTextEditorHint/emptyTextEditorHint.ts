...
class EmptyTextEditorHintContentWidget extends Disposable implements IContentWidget {
	...
	private getHint() {
		...
		const keybindingLabels = [ChangeLanguageAction.ID].map(id => this.keybindingService.lookupKeybinding(id)?.getLabel());

		const hintMsg = localize({
			key: 'emptyTextEditorHintWithoutInlineAssist',
			comment: [
		...
		const ariaLabel = localize('defaultHintAriaLabelWithoutInlineAssist', 'Execute {0} to select a language and get started. Start typing to dismiss.', ...keybindingLabels);
