...
import { IWorkbenchThemeService, IWorkbenchColorTheme, ThemeSettingDefaults } from '../../../services/themes/common/workbenchThemeService.js';
...
class Snapper {
	...
	private async _getThemesResult(grammar: IGrammar, lines: string[]): Promise<IThemesResult> {
		...
		const getThemeName = (settingsId: string) => {
			if (settingsId === ThemeSettingDefaults.COLOR_THEME_DARK) {
				return 'dark';
			}
			if (settingsId === ThemeSettingDefaults.COLOR_THEME_LIGHT) {
				return 'light';
			}
			return undefined;
		};
		...
		const defaultThemes = themeDatas.filter(themeData => !!getThemeName(themeData.settingsId));
		for (const defaultTheme of defaultThemes) {
			const themeId = defaultTheme.id;
			const success = await this.themeService.setColorTheme(themeId, undefined);
			if (success) {
				const themeName = getThemeName(defaultTheme.settingsId);
				result[themeName!] = {
