	...
	properties: {
		engines: {
			...
			properties: {
				'vscode': {
					type: 'string',
					description: nls.localize('vscode.extension.engines.vscode', 'For KnoxCoder extensions, specifies the KnoxCoder version that the extension is compatible with. Cannot be *. For example: ^1.105.0 indicates compatibility with a minimum KnoxCoder version of 1.105.0.'),
					default: '^1.105.0',
				}
			}
		},
		publisher: {
			description: nls.localize('vscode.extension.publisher', 'The publisher of the KnoxCoder extension.'),
			type: 'string'
		},
		displayName: {
			description: nls.localize('vscode.extension.displayName', 'The display name for the extension used in the KnoxCoder gallery.'),
			type: 'string'
		},
		categories: {
			description: nls.localize('vscode.extension.categories', 'The categories used by the KnoxCoder gallery to categorize the extension.'),
			type: 'array',
			uniqueItems: true,
			items: {
				oneOf: [{
		...
		galleryBanner: {
			type: 'object',
			description: nls.localize('vscode.extension.galleryBanner', 'Banner used in the KnoxCoder marketplace.'),
			properties: {
				color: {
					description: nls.localize('vscode.extension.galleryBanner.color', 'The banner color on the KnoxCoder marketplace page header.'),
					type: 'string'
				},
				theme: {
		...
		contributes: {
			description: nls.localize('vscode.extension.contributes', 'All contributions of the KnoxCoder extension represented by this package.'),
			type: 'object',
		...
		activationEvents: {
			description: nls.localize('vscode.extension.activationEvents', 'Activation events for the KnoxCoder extension.'),
			type: 'array',
			items: {
				type: 'string',
				defaultSnippets: [
					...
					{
						label: 'onAssistParticipant',
						body: 'onAssistParticipant:${1:participantId}',
						description: nls.localize('vscode.extension.activationEvents.onAssistParticipant', 'An activation event emitted when the specified assist participant is invoked.'),
					},
					{
						label: 'onAssistContextProvider',
						body: 'onAssistContextProvider:${1:contextProviderId}',
						description: nls.localize('vscode.extension.activationEvents.onAssistContextProvider', 'An activation event emitted when the specified assist context provider is invoked.'),
					},
					{
						label: 'onTextModelApiAssistProvider',
						body: 'onTextModelApiAssistProvider:${1:vendor}',
						description: nls.localize('vscode.extension.activationEvents.onTextModelApiAssistProvider', 'An activation event emitted when a assist model provider for the given vendor is requested.'),
					},
					{
						label: 'onTextModelApiTool',
						body: 'onTextModelApiTool:${1:toolId}',
						description: nls.localize('vscode.extension.activationEvents.onTextModelApiTool', 'An activation event emitted when the specified language model tool is invoked.'),
					},
					{
						label: 'onTerminal',
					...
					{
						label: 'onToolCollection',
						description: nls.localize('vscode.extension.activationEvents.onToolCollection', 'An activation event emitted whenever a tool from the tool server is requested.'),
						body: 'onToolCollection:${2:collectionId}',
					},
					{
						label: '*',
						description: nls.localize('vscode.extension.activationEvents.star', 'An activation event emitted on KnoxCoder startup. To ensure a great end user experience, please use this activation event in your extension only when no other activation events combination works in your use-case.'),
						body: '*'
					}
				],
			}
		...
		scripts: {
			...
			properties: {
				'vscode:prepublish': {
					description: nls.localize('vscode.extension.scripts.prepublish', 'Script executed before the package is published as a KnoxCoder extension.'),
					type: 'string'
				},
				'vscode:uninstall': {
					description: nls.localize('vscode.extension.scripts.uninstall', 'Uninstall hook for KnoxCoder extension. Script that gets executed when the extension is completely uninstalled from KnoxCoder which is when KnoxCoder is restarted (shutdown and start) after the extension is uninstalled. Only Node scripts are supported.'),
					type: 'string'
				}
			}
		},
