...
function validateThemeId(theme: string): string {
	// migrations
	switch (theme) {
		case ThemeTypeSelector.VS: return `vs ${defaultThemeExtensionId}-themes-light-json`;
		case ThemeTypeSelector.VS_DARK: return `vs-dark vscode-theme-onedark-pro-themes-dark-json`;
		case ThemeTypeSelector.HC_BLACK: return `vs-dark vscode-theme-onedark-pro-themes-dark-json`;
		case ThemeTypeSelector.HC_LIGHT: return `vs ${defaultThemeExtensionId}-themes-light-json`;
	}
	return theme;
}
...
export class WorkbenchThemeService extends Disposable implements IWorkbenchThemeService {
	...
	private async showNewDefaultThemeNotification(previousSettingsId: string | undefined): Promise<void> {
		...
		try {
			...
			const migratedPreviousSettingsId = migrateThemeSettingsId(previousSettingsId);
			if (migratedPreviousSettingsId === previousSettingsId) {
				return;
			}
