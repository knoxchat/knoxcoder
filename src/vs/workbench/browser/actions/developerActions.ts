...
/** Policy diagnostics reduced after chat/agent managed-settings removal. */
class PolicyDiagnosticsAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		let content = '# Policy Diagnostics\n\n';
		content += 'Chat/agent managed-settings diagnostics were removed from this product.\n\n';
		content += `Policy service keys: ${Object.keys(policyService.serialize() ?? {}).length}\n`;
		await editorService.openEditor({ resource: undefined, contents: content, languageId: 'markdown' });
	}
}
