...
interface IEmbeddingsService {
	...
	allProviders: IterableIterator<string>;
...
class EmbeddingsService implements IEmbeddingsService {
	...
	get allProviders(): IterableIterator<string> {
...
export class MainThreadEmbeddings implements MainThreadEmbeddingsShape {
	...
	private readonly _providers = this._store.add(new DisposableMap<number, IDisposable>());
