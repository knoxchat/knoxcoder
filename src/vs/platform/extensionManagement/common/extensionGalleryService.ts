...
function getVersionAsset(version: IRawGalleryExtensionVersion, type: string): IGalleryExtensionAsset | null {
	...
	const constructedUri = `${version.assetUri}/${type}${version.targetPlatform ? `?targetPlatform=${version.targetPlatform}` : ''}`;
	const constructedFallbackUri = `${version.fallbackAssetUri}/${type}${version.targetPlatform ? `?targetPlatform=${version.targetPlatform}` : ''}`;
	if (result.source) {
		return { uri: result.source, fallbackUri: constructedFallbackUri };
	}
	return { uri: constructedUri, fallbackUri: constructedFallbackUri };
}
...
export abstract class AbstractExtensionGalleryService implements IExtensionGalleryService {
	...
	private async queryRawGalleryExtensions(query: Query, extensionGalleryManifest: IExtensionGalleryManifest, token: CancellationToken): Promise<IRawGalleryExtensionsResult> {
		...
		// Open VSX does not use the Microsoft Marketplace target filter.
		if (!this.productService.extensionsGallery?.serviceUrl?.includes('open-vsx.org')) {
			query = query.withFilter(FilterType.Target, 'Microsoft.VisualStudio.Code');
		}
	...
	async getReadme(extension: IGalleryExtension, token: CancellationToken): Promise<string> {
		if (extension.assets.readme) {
			...
			if (!content?.trim() && extension.assets.readme.fallbackUri !== extension.assets.readme.uri) {
				const fallbackContext = await this.requestService.request({
					type: 'GET',
					url: extension.assets.readme.fallbackUri,
					timeout: this.getRequestTimeout(),
					followRedirects: 5,
					callSite: 'extensionGalleryService.readme.fallback',
				}, token);
				const fallbackContent = await asTextOrError(fallbackContext);
	...
	private async getAsset(extension: string, asset: IGalleryExtensionAsset, assetType: string, extensionVersion: string, callSite: string, options: Omit<IRequestOptions, 'callSite'> = {}, token: CancellationToken = CancellationToken.None): Promise<IRequestContext> {
		...
		const firstOptions = { ...options, url, timeout: this.getRequestTimeout(), followRedirects: 5, callSite };
		...
		try {
			...
			const fallbackOptions = { ...options, url: fallbackUrl, timeout: this.getRequestTimeout(), followRedirects: 5, callSite: `${callSite}.fallback` };
