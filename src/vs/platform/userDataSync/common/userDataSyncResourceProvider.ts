...
export class UserDataSyncResourceProviderService implements IUserDataSyncResourceProviderService {
	...
	async getAssociatedResources({ uri }: ISyncResourceHandle): Promise<{ resource: URI; comparableResource: URI }[]> {
		...
		switch (resolved.syncResource) {
			...
			case SyncResource.Tasks: return this.getTasksAssociatedResources(uri, profile);			case SyncResource.Snippets: return this.getSnippetsAssociatedResources(uri, profile);
	...
	private resolveNodeContent(syncResource: SyncResource, content: string, node: string): string | null {
		...
		switch (syncResource) {
			...
			case SyncResource.Tasks: return this.resolveTasksNodeContent(syncData, node);			case SyncResource.Snippets: return this.resolveSnippetsNodeContent(syncData, node);
