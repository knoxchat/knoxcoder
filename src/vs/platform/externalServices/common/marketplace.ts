...
export async function resolveMarketplaceHeaders(version: string,
	...
	const extensionsGallery = productService.extensionsGallery as { authorizationHeaderToken?: string } | undefined;
	if (extensionsGallery?.authorizationHeaderToken) {
		headers['Authorization'] = `Bearer ${extensionsGallery.authorizationHeaderToken}`;
	}
