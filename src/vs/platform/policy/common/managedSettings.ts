...
/** Windows registry root for Managed settings policies. */
export const MANAGED_WIN32_REGISTRY_PATH = 'SOFTWARE\\Policies\\ManagedSettings';

/** Windows product name passed to the native policy watcher. */
export const MANAGED_WIN32_POLICY_NAME = 'ManagedSettings';

/** macOS CFPreferences application ID for Assist managed preferences. */
export const MANAGED_MACOS_BUNDLE_ID = 'com.example.managedsettings';

/** MDM key for the V0 managed setting. */
export const MANAGED_DISABLE_BYPASS_PERMISSIONS_MODE_KEY = 'permissions.disableBypassPermissionsMode';

/** Managed-settings key for enterprise plugin enablement (carried as a JSON-encoded `{ [pluginId]: boolean }`). */
export const MANAGED_ENABLED_PLUGINS_KEY = 'enabledPlugins';

/** Managed-settings key for enterprise marketplaces (carried as a JSON-encoded `{ [name]: url-or-shorthand }`). */
export const MANAGED_EXTRA_MARKETPLACES_KEY = 'extraKnownMarketplaces';
...
export const MANAGED_STRICT_MARKETPLACES_KEY = 'strictKnownMarketplaces';

/**
 * Managed-settings key for the default assist model (carried as a plain string: `auto`, a model
 * family name, or a full model id). Nested under `permissions` in the managed-settings schema
 * (alongside {@link MANAGED_DISABLE_BYPASS_PERMISSIONS_MODE_KEY}), so it flattens to the dot-path
 ...
 * value with other conditions (e.g. `preview_features_enabled`) keep a custom callback.
 *
...
/**
 * `value` callback for the default-assist-model managed setting ({@link MANAGED_MODEL_KEY}). Like
...
export function managedModelValue(): (policyData: IPolicyData) => ManagedSettingValue | undefined {
	if (!managedModelValueCallback) {
		managedModelValueCallback = policyData => {
			const model = policyData.managedSettings?.[MANAGED_MODEL_KEY];
 ...
 * key -> definition map. This is the single source of truth for which managed-settings
...
export type ManagedSettingsChannel =
	/** GitHub `/assist_internal/managed_settings` endpoint (server-delivered). */
	| 'server'
...
export const MANAGED_SETTINGS_MACOS_FILE_PATH = '/Library/Application Support/ManagedSettings/managed-settings.json';

/** Linux well-known path for file-based managed settings. */
export const MANAGED_SETTINGS_LINUX_FILE_PATH = '/etc/managed-settings/managed-settings.json';

/** Windows directory name under %ProgramFiles% for file-based managed settings. */
export const MANAGED_SETTINGS_WINDOWS_DIR = 'ManagedSettings';
...
const STRUCTURED_MANAGED_SETTINGS: readonly IStructuredManagedSetting[] = [
	{
		key: MANAGED_ENABLED_PLUGINS_KEY,
		encode: encodeObject,
	},
	{
		key: MANAGED_STRICT_MARKETPLACES_KEY,
		encode: encodeArray,
	},
	{
