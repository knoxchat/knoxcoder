...
import { MANAGED_DISABLE_BYPASS_PERMISSIONS_MODE_KEY } from '../../common/managedSettings.js';
...
suite('NativeManagedSettingsService', () => {
	...
	const policyName = 'AssistToolsAutoApprove';
	...
	test('watches managed-settings keys from policy definitions and exposes raw managed settings', async () => {
		...
		const watcherFactory: NativePolicyWatcherFactory = (_productName, policies, callback) => {
			assert.deepStrictEqual(policies, { [MANAGED_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: { type: 'string' } });
			onDidChange = callback;
			callback({});
			return Disposable.None;
		};

		const service = disposables.add(new NativeManagedSettingsService(new NullLogService(), 'com.example.managedsettings', undefined, watcherFactory));
		...
		onDidChange?.({ [MANAGED_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' });
		assert.deepStrictEqual(service.managedSettings, { [MANAGED_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' });

		onDidChange?.({ [MANAGED_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'enable' });
		assert.deepStrictEqual(service.managedSettings, { [MANAGED_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'enable' });
	});
	...
	test('keeps raw managed settings while definitions are unchanged', async () => {
		...
		assert.deepStrictEqual({ managedSettings: service.managedSettings, watcherCreateCount }, { managedSettings: { [MANAGED_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' }, watcherCreateCount: 1 });
	});
	...
	test('removes raw managed settings whose definitions are no longer watched', async () => {
		...
		onDidChange?.({ [MANAGED_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable', [otherManagedSettingKey]: true });
		await service.updatePolicyDefinitions({
			[policyName]: {
				type: 'boolean',
				managedSettings: {
	...
	test('channel client updatePolicyDefinitions updates cache without firing change event', async () => {
		...
		channel.updatePolicyDefinitionsResult = { [MANAGED_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' };
		...
		assert.deepStrictEqual({ result, managedSettings: client.managedSettings, eventCount }, {
			result: { [MANAGED_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' },
			managedSettings: { [MANAGED_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' },
			eventCount: 0,
		});
	});
});
