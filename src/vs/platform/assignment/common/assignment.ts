...
export enum Filters {
	...
	/**
	 * The platform (OS) on which KnoxCoder is running.
	 */
	Platform = 'X-VSCode-Platform',

	/**
	 * The release/build date of KnoxCoder (UTC) in the format yyyymmddHH.
	 */
	ReleaseDate = 'X-VSCode-ReleaseDate',

	/**
	 * The kind of window KnoxCoder is running in (`editor` or `agents`).
	 */
...
export function getInternalOrg(organisations: string[] | undefined): 'vscode' | 'github' | 'microsoft' | undefined {
	...
	const isMicrosoftInternal = organisations?.includes('microsoft') || organisations?.includes('ms-assist') || organisations?.includes('MicrosoftAssist');
