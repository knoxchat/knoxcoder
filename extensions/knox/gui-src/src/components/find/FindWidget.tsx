...
export const useFindWidget = (searchRef: RefObject<HTMLDivElement>) => {
  ...
  const refreshSearch = useCallback(
    (scrollTo: ScrollToMatchOption = "none", clearFirst = false) => {
      ...
      const pattern: SearchPattern = compileSearchPattern(_query, {
        caseSensitive,
        useRegex,
      });
        ...
        NodeFilter.SHOW_ALL,
        {
          acceptNode: (node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              if (textMatchesPattern(node.nodeValue, pattern)) {
                return NodeFilter.FILTER_ACCEPT;
              }
            }
            return NodeFilter.FILTER_REJECT;
      ...
      textNodes.forEach((textNode) => {
        ...
        const ranges = findMatchRanges(nodeText, pattern);
      ...
      if (_query.length > 1 && filteredMatches.length) {
        if (scrollTo === "first") {
    ...
    return (
      ...
      >
        ...
        <p className="xs:block hidden min-w-12 whitespace-nowrap px-1 text-center text-xs">
          {matches.length === 0
            ? t('noResults')
            : t('matchCount', { current: (currentMatch?.index ?? 0) + 1, total: matches.length })}
        </p>
        <div className="hidden flex-row gap-0.5 sm:flex">
          <HeaderButtonWithToolTip
            tooltipPlacement="top-end"
            text={t('previousMatch')}
            onClick={(e) => {
              e.stopPropagation();
              previousMatch();
            }}
            className="h-4 w-4 focus:ring-3"
            disabled={matches.length < 2 || active}
          >
            <ArrowUp className="text-knoxcyan h-4 w-4" />
          </HeaderButtonWithToolTip>
          <HeaderButtonWithToolTip
            tooltipPlacement="top-end"
            text={t('nextMatch')}
            ...
            disabled={matches.length < 2 || active}
          >
            <ArrowDown className="text-knoxcyan h-4 w-4" />
          </HeaderButtonWithToolTip>
        </div>
        <HeaderButtonWithToolTip
          disabled={active}
          ...
          className="h-5 w-6 rounded-full border text-xs focus:outline-hidden focus:ring-3"
        >
          Aa
        </HeaderButtonWithToolTip>
        <HeaderButtonWithToolTip
          ...
          inverted={useRegex}
          tooltipPlacement="top-end"
          text={useRegex ? t("closeRegex") : t("openRegex")}
          onClick={(e) => {
            e.stopPropagation();
            setUseRegex((curr) => !curr);
          }}
          className="h-5 w-6 rounded-full border text-xs focus:outline-hidden focus:ring-3"
        >
          .*
