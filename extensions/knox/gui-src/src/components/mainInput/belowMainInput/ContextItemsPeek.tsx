...
function ContextItemsPeekItem({ contextItem }: ContextItemsPeekItemProps) {
  ...
  function openContextItem() {
    ...
    if (isUrl) {
      if (uri?.value) {
        ...
        console.error("Can not open URL", uri);
      }
    } else if (uri) {
  ...
  function getContextItemIcon() {
    ...
    if (contextItem.icon) {
      ...
      return (
        <SafeImg
          className="mr-2 shrink-0 rounded-md p-1"
  ...
  return (
    <div
      ...
      className="group mr-2 flex cursor-pointer items-center overflow-hidden text-ellipsis whitespace-nowrap rounded-sm px-1.5 py-1 text-xs hover:bg-white/10"
    ...
    >
      <div className="flex w-full items-center">
        ...
        <div className="flex min-w-0 flex-1 gap-2 text-xs">
          <div
            className={`max-w-[50%] shrink-0 truncate ${isUrl ? "hover:underline" : ""}`}
          >
            {contextItem.name}
          </div>
          <div
            className={`min-w-0 flex-1 overflow-hidden truncate whitespace-nowrap text-xs text-knoxcyan ${isUrl ? "hover:underline" : ""}`}
          >
        ...
        {isUrl && (
          <ArrowUpRight className="mx-2 h-4 w-4 shrink-0 text-knoxcyan opacity-0 group-hover:opacity-100" />
        )}
      </div>
    </div>
  );
...
function ContextItemsPeek({
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative mr-1 h-3.5 w-3.5">
          <ChevronRight
            className={`text-knoxcyan absolute h-4 w-4 transition-all duration-200 ease-in-out text-[${lightGray}] ${
              open ? "rotate-90 opacity-0" : "rotate-0 opacity-100"
            }`}
          />
          <ChevronDown
            className={`text-knoxcyan absolute h-3.5 w-3.5 transition-all duration-200 ease-in-out text-[${lightGray}] ${
              open ? "rotate-0 opacity-100" : "-rotate-90 opacity-0"
            }`}
          />
        </div>
        <span className="text-knoxcyan ml-1 text-xs transition-colors duration-200">
          {isGatheringContext ? (
            <>
              {t('gatheringContext')}
              <AnimatedEllipsis />
            ...
            t('relatedContextItems', { count: ctxItems.length })
          )}
        </span>
      </div>
