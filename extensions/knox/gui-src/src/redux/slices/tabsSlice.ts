...
export const tabsSlice = createSlice({
    ...
    reducers: {
        ...
        addTab: (state, action: PayloadAction<Tab>) => {
            state.tabs = state.tabs.map(tab => ({ ...tab, isActive: action.payload.isActive ? false : tab.isActive }))
        ...
        handleSessionChange: (
            ...
            if (!activeTab.sessionId) {
                ...
                state.tabs = state.tabs.map(tab => ({ ...tab, isActive: false })).concat({
                    id: newTabId || Date.now().toString(36) + Math.random().toString(36).substring(2),
                    title: currentSessionTitle,
                    isActive: true,
                    sessionId: currentSessionId,
                });
