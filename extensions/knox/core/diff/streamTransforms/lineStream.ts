...
export async function* noTopLevelKeywordsMidline(
  ...
  for await (const line of lines) {
    ...
    for (const keyword of topLevelKeywords) {
      ...
      // Only truncate when the keyword is glued mid-token (e.g. "xdef "), not
      // when it starts the line or follows whitespace (a real top-level / indented keyword).
      if (
        indexOf > 0 &&
        line.charAt(indexOf - 1).trim() !== ""
      ) {
