...
import { LANGUAGES } from "../../util/languageInfo";
...
/** Reject diffs that delete a suspiciously large fraction of the file. */
...
/** True when lazy placeholders were not expanded out of the reconstruction. */
function stillContainsLazyBlocks(text: string): boolean {
  return text.split("\n").some((line) => isLazyLine(line));
}
...
/**
 * Deterministic (AST) lazy apply. Returns `undefined` when confidence is low
 * so callers can fall back to unified-diff or LLM lazy apply.
 */
export async function deterministicApplyLazyEdit(
  ...
  const hadLazyBlocks = Boolean(findInAst(newTree.rootNode, isLazyBlock));

  // Full-file rewrite with no lazy placeholders — high confidence if the diff
  // is not dominated by deletions.
  if (!hadLazyBlocks) {
    // First, we need to check whether there are matching (similar) nodes at the root level
    const firstChild = newTree.rootNode.children[0];
    const firstSimilarNode = firstChild
      ? findInAst(oldTree.rootNode, (node) => nodesAreSimilar(node, firstChild))
      : null;
    if (firstSimilarNode?.parent?.equals(oldTree.rootNode)) {
      ...
      if (result) {
        ...
        // Language has no comment syntax in our table — treat as full rewrite
        reconstructedNewFile = newLazyFile;
      }
    } else {
      // If not, we need to recursively search for the nodes that are being rewritten,
      // and we apply a slightly different algorithm
      const currentNewTree = newTree; // Type narrowing helper
      const newCodeNumLines = currentNewTree.rootNode.text.split("\n").length;
      const matchingNode = findInAst(
        oldTree.rootNode,
        (node) => programNodeIsSimilar(currentNewTree.rootNode, node),
      ...
      if (matchingNode) {
        ...
        reconstructedNewFile =
          ...
          currentNewTree.rootNode.text +
          oldText.slice(endIndex);
      } else if (
        newLazyFile.trim().length > 0 &&
        oldFile.trim().length > 0 &&
        newLazyFile.split("\n").length >= oldFile.split("\n").length * 0.5
      ) {
        // Likely a full-file rewrite with no structural match — accept if size is plausible
        reconstructedNewFile = newLazyFile;
      } else {
        console.warn(t("noNodeMatchingLazyBlock"));
        return undefined;
      }
    }
  }
  ...
  if (!reconstructedNewFile) {
    ...
    // Lazy blocks present but no AST replacements found → low confidence
    if (hadLazyBlocks && replacements.length === 0) {
      return undefined;
    }
  ...
  // Failed to expand placeholders → fall back
  if (stillContainsLazyBlocks(reconstructedNewFile)) {
    return undefined;
  }
...
function isLazyBlock(node: Node): boolean {
  // Special case for "{/* ... existing code ... */}"
  if (
    node.type === "jsx_expression" &&
    node.namedChildCount === 1 &&
    isLazyBlock(node.namedChildren[0] as Node)
  ) {
    return true;
  }
 ...
 * @returns
 */
function nodesAreSimilar(a: Node, b: Node): boolean {
  if (a.type !== b.type) {
    return false;
  }
...
function nodesAreExact(a: Node, b: Node): boolean {
...
function findLazyBlockReplacements(
  ...
  const leftChildren = oldNode.namedChildren.filter((n): n is Node => n !== null);
  const rightChildren = newNode.namedChildren.filter((n): n is Node => n !== null);
  let isLazy = false;
  let currentLazyBlockNode: Node | undefined = undefined;
  const currentLazyBlockReplacementNodes: Node[] = [];
