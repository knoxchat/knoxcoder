...
class GreptileContextProvider extends BaseContextProvider {
  static description: ContextProviderDescription = {
    ...
    description: t("insertQueryToGreptile"),
    type: "query",
  };
  ...
  async getContextItems(
    ...
    if (!greptileToken) {
      throw new Error(t("greptileTokenNotFound"));
    }

    const githubToken = this.getGithubToken();
    if (!githubToken) {
      throw new Error(t("githubTokenNotFound"));
    }

    let absPath = await this.getWorkspaceDir(extras);
    if (!absPath) {
      throw new Error(t("unableToDetermineWorkspaceDir"));
    }
    ...
    if (!remoteType) {
      throw new Error(t("unableToDetermineRemoteType"));
    }
    ...
    try {
      ...
      console.error(t("errorFetchingGreptileContext"), error);
      throw new Error(t("errorFetchingGreptileContext"));
    }
  }
  ...
  private async getWorkspaceDir(
    ...
    try {
      ...
      if (workspaceDirs && workspaceDirs.length > 0) {
        ...
        console.warn(t("workspaceDirsEmpty"));
      }
    } catch (err) {
      console.warn(t("unableToGetWorkspaceDirFromIde"));
    }
    ...
    try {
      ...
      if (this.isGitRepository(currentDir)) {
        ...
        console.warn(t("currentDirNotGitRepo", { dir: currentDir }));
        return null;
      }
    } catch (err) {
      console.warn(t("unableToGetWorkspaceDirGit"));
      return null;
    }
  }
  ...
  private isGitRepository(dir: string): boolean {
    try {
      ...
      console.warn(t("unableToCheckGitRepo"));
      return false;
    }
  }
}
...
function getRemoteUrl(absPath: string): string {
  try {
    ...
    console.warn(t("unableToGetRemoteUrl"));
    return "";
  }
}
...
function getCurrentBranch(absPath: string): string {
  try {
    ...
    console.warn(t("unableToGetCurrentBranch"));
