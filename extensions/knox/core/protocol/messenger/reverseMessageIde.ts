...
export class ReverseMessageIde {
  ...
  private initializeListeners() {
    ...
    this.on("debugControl", (data) => {
      if (typeof this.ide.debugControl !== "function") {
        return {
          ok: false,
          content: "No DAP backend.",
          sessionActive: false,
        };
      }
      return this.ide.debugControl(data);
    });
    ...
    this.on("getGitChangedFiles", () => {
      return this.ide.getGitChangedFiles();
    });
    ...
    this.on("getSearchResults", (data) => {
      return this.ide.getSearchResults(data.query, data.options);
    });
