...
function parseDataLine(line: string): any {
  ...
  if (data.error) {
    throw new Error(t("errorStreamingResponse", { error: data.error }));
  }
...
export function parseSseLine(line: string): { done: boolean; data: any } {
  ...
  try {
    ...
    if (error instanceof Error && error.message.startsWith("Error streaming")) {
      throw error;
    }
    // A single malformed data line must not abort the rest of the stream.
    return { done: false, data: undefined };
  }
}
