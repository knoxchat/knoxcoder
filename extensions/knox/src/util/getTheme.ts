...
export function getTheme() {
  ...
  try {
          ...
          if (theme.id === colorTheme || theme.label === colorTheme) {
            ...
            while (currentParsedTheme.include) {
              ...
              if (fs.existsSync(includeThemePath)) {
                ...
                console.log(
                  `Could not find included theme ${currentTheme} in ${themeDir} looking for ${currentParsedTheme.include}`,
                  includeThemePath,
                );
                break;
              }
    ...
    if (!currentTheme) {
      console.warn(`Could not find any theme file for theme ${colorTheme}`);
      return undefined;
    }
