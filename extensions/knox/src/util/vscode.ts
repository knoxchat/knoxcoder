...
/** System extension id in KnoxCoder (`publisher.name`). */
export const KNOX_EXTENSION_ID = "vscode.knox";
/** Marketplace id on Open VSX. KnoxCoder disables and hides this when `vscode.knox` is present (T6.3). */
export const KNOX_MARKETPLACE_EXTENSION_ID = "knoxchat.knoxchat";

export function getKnoxExtension(): vscode.Extension<unknown> | undefined {
  return (
    vscode.extensions.getExtension(KNOX_EXTENSION_ID) ??
    vscode.extensions.getExtension(KNOX_MARKETPLACE_EXTENSION_ID)
  );
}
...
export function getExtensionUri(): vscode.Uri {
  return getKnoxExtension()!.extensionUri;
}
