...
export class QuickEdit {
  ...
  async show(params?: QuickEditShowParams) {
    // T7.3: indexing is deferred from activate; kick it off when the command runs.
  ...
  private openAcceptRejectMenu(prompt: string, path: string | undefined) {
    ...
    quickPick.placeholder = "Please enter your accept decision";
    quickPick.title = "Accept Changes";
    quickPick.value = prompt;
    quickPick.items = REVIEW_CHANGES_ITEMS;
    quickPick.placeholder =
      "Accept or reject changes, or enter a new prompt to retry.";
    quickPick.show();
    ...
    quickPick.onDidAccept(async () => {
      ...
      switch (label) {
        case UserPromptLabels.AcceptAll:
          vscode.commands.executeCommand("knoxchat.acceptDiff", path);
          break;
        case UserPromptLabels.RejectAll:
          vscode.commands.executeCommand("knoxchat.rejectDiff", path);
          break;
        case QuickEditInitialItemLabels.Submit:
          if (quickPick.value) {
            await vscode.commands.executeCommand("knoxchat.rejectDiff", path);
    ...
    return [
      {
        ...
        detail: "$(history) Select previous prompt",
      },
      {
        label: QuickEditInitialItemLabels.ContextProviders,
        detail: "$(add) Add context to prompt",
      },
      {
  ...
  private handleQuickPickAccept = ({
    ...
    // If not an initial item, it's a file selection. Allow prompt editing.
